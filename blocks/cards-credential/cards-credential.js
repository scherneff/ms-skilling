...
function buildLinksCard(article) {
  ...
  const link = createTag('a', { href, class: 'cards-credential-card-link' });

  if (article.image) {
    const imageDiv = createTag('div', { class: 'cards-credential-card-image' });
    imageDiv.append(createOptimizedPicture(article.image, article.title || '', false, [{ width: '750' }]));
    link.append(imageDiv);
  }

  const body = createTag('div', { class: 'cards-credential-card-body' });
...
async function decorateLinks(block) {
  ...
  if (!authoredLinks.length) {
    block.textContent = '';
    block.append(createTag('p', { class: 'cards-credential-links-empty' }, 'No links provided.'));
    return;
  }
...
function decorateBento(block) {
  ...
  [...block.children].forEach((row, idx) => {
    const li = createTag('li');
    if (idx === 0) li.classList.add('cards-credential-card-featured');
    ...
    if (picture) {
      ...
      li.classList.add('cards-credential-card-text-only');
    }
    ...
    if (firstP && !firstP.querySelector('picture') && !firstP.classList.contains('button-container')) {
      firstP.classList.add('cards-credential-card-tag');
    }

    // Wrap remaining non-image content in a body div
    const body = createTag('div', { class: 'cards-credential-card-body' });
    [...li.children].forEach((child) => {
      if (!child.classList.contains('cards-credential-card-image')) body.append(child);
    });
    li.append(body);
  ...
  [...block.children].forEach((row) => {
    ...
    if (content?.children?.length > 1) {
      ...
      if (imageEl) {
        ...
        const bodyDiv = createTag('div', { class: 'cards-credential-card-body' });
        [...content.children].forEach((el) => { if (el !== imageEl) bodyDiv.append(el); });
        li.replaceChildren(imageDiv, bodyDiv);
      } else {
        content.className = 'cards-credential-card-body';
      }
    } else {
      [...li.children].forEach((div) => {
        div.className = (div.children.length === 1 && div.querySelector('picture'))
          ? 'cards-credential-card-image' : 'cards-credential-card-body';
      });
    }

    const linkEl = li.querySelector('.cards-credential-card-image a[href]') || li.querySelector('.cards-credential-card-body a[href]');
    if (linkEl) {
      if (isUE) {
        // In UE: use a <div> wrapper so the authored <a> (with its href) is preserved
        const wrapper = createTag('div', { class: 'cards-credential-card-link' });
        ...
        const wrapper = createTag('a', {
          ...
          class: 'cards-credential-card-link',
        });
        while (li.firstChild) wrapper.append(li.firstChild);
        li.append(wrapper);
        linkEl.replaceWith(...linkEl.childNodes);
        li.querySelectorAll('.cards-credential-card-body a[href]').forEach((a) => a.replaceWith(...a.childNodes));
      }
    }
