...
function findTabGroups(main) {
  const sections = [...main.querySelectorAll('.section[data-tab-id]')]
    .filter((s) => !s.closest('.tabs-audience-wrapper'));
...
function buildTabsUI(tabDefs, tabsContainer, sectionId = '') {
  ...
  tabsWrapper.className = 'tabs-audience-wrapper';
  ...
  tabContent.className = 'tabs-audience-content';
  ...
  tabDefs.forEach((tabDef) => {
    ...
    button.className = 'tabs-audience-tab';
    button.id = buttonId;
    button.type = 'button';
    button.role = 'tab';
